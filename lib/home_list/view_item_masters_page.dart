...
class _ViewItemsOldPageState extends State<ViewItemsOldPage> {
  ...
  setValueLoadItemMaster(String? key) async {
    ...
    try {
      for (var i = 0; i < 15; i++) {
        print(itemMasters[i]);
        widget.transactionType == "PO" && itemMasters[i]['ItemPOBlocked'] != 1
            ? itemMastersLists.add(itemMasters[i])
            : (widget.transactionType == "MJ" ||
                        widget.transactionType == "TO") &&
                    itemMasters[i]['ItemInventBlocked'] != 1
                ? itemMastersLists.add(itemMasters[i])
                : itemMastersLists.add(itemMasters[i]);
      }
      setState(() {
  ...
  getUserData() async {
    prefs = await SharedPreferences.getInstance();
    await prefs!.setBool("lineDeleted", false) ?? false;
            ...
            TextFormField(
              onFieldSubmitted: (value) async {
                var result = await _sqlHelper
                    .getItemMatersBySearch(searchController.text.trim());
              ...
              decoration: InputDecoration(
                ...
                prefixIcon: IconButton(
                  ...
                  onPressed: () async {
                    ...
                    var result = await _sqlHelper
                        .getItemMatersBySearch(searchController.text);
            ...
            isLoading
                ...
                : Expanded(
                    ...
                    child: SmartRefresher(
                      ...
                      enablePullDown:
                          itemMasters.length != itemMastersLists.length
                              ? true
                              : false,
                      enablePullUp: true,
                        ...
                        builder: (BuildContext? context, LoadStatus? mode) {
                          ...
                          if (mode == LoadStatus.idle) {
                            ...
                            if (itemMastersLists.length <= itemMasters.length) {
                              ...
                              itemMastersLists.length != itemMasters.length) {
                                              ...
                                              if (widget.transactionType ==
                                                ...
                                                if (dt.length > 0) {
                                                  ...
                                                  await _sqlHelper.addTRANSDETAILS(
                                                      ...
                                                      QTY: 1,
                                                      UOM: itemMastersLists[index]
                                                          ['UOM'],
                                                      BARCODE: itemMastersLists[index]
                                                          ['BARCODE'],
                                                ...
                                                print(
                                                    "Line mj 1177 : ${widget.transactionType.toString()}");
    ...
    for (var i = itemMastersLists.length - 1; i < itemMasters.length - 1; i++) {
      ...
      if (u < 4) {
        ...
        if (itemMastersLists.length <= itemMasters.length) {
          ...
          itemMastersLists.length <= itemMasters.length &&
                  itemMasters[i + 1] != null
              ? itemMastersLists.add(itemMasters[i + 1])
              : null;

          itemMastersLists.length <= itemMasters.length &&
                  itemMasters[i + 2] != null
              ? itemMastersLists.add(itemMasters[i + 2])
              : null;

          itemMastersLists.length <= itemMasters.length &&
                  itemMasters[i + 3] != null
              ? itemMastersLists.add(itemMasters[i + 3])
              : null;

          itemMastersLists.length <= itemMasters.length &&
                  itemMasters[i + 4] != null
              ? itemMastersLists.add(itemMasters[i + 4])
              : null;

          itemMastersLists.length <= itemMasters.length &&
                  itemMasters[i + 5] != null
              ? itemMastersLists.add(itemMasters[i + 5])
              : null;

          itemMastersLists.length <= itemMasters.length &&
                  itemMasters[i + 6] != null
              ? itemMastersLists.add(itemMasters[i + 6])
              : null;

          itemMastersLists.length <= itemMasters.length &&
                  itemMasters[i + 7] != null
              ? itemMastersLists.add(itemMasters[i + 7])
              : null;

          itemMastersLists.length <= itemMasters.length &&
                  itemMasters[i + 8] != null
              ? itemMastersLists.add(itemMasters[i + 8])
              : null;

          itemMastersLists.length <= itemMasters.length &&
                  itemMasters[i + 9] != null
              ? itemMastersLists.add(itemMasters[i + 9])
              : null;

          itemMastersLists.length <= itemMasters.length &&
                  itemMasters[i + 10] != null
              ? itemMastersLists.add(itemMasters[i + 10])
              : null;

          itemMastersLists.length <= itemMasters.length &&
                  itemMasters[i + 11] != null
              ? itemMastersLists.add(itemMasters[i + 11])
              : null;

          itemMastersLists.length <= itemMasters.length &&
                  itemMasters[i + 12] != null
              ? itemMastersLists.add(itemMasters[i + 12])
              : null;

          itemMastersLists.length <= itemMasters.length &&
                  itemMasters[i + 13] != null
              ? itemMastersLists.add(itemMasters[i + 13])
              : null;

          itemMastersLists.length <= itemMasters.length &&
                  itemMasters[i + 14] != null
              ? itemMastersLists.add(itemMasters[i + 14])
              : null;
        } else {}
  ...
  setvalue() async {
    ...
    // list1.addAll(await _sqlHelper
    //     .getItemMaters(itemMastersLists[itemMastersLists.length - 1]['id']));
    // list2.addAll(await _sqlHelper
    //     .getItemMaters(itemMastersLists[itemMastersLists.length - 1]['id']));
    // setState((){
    //
    // });
    ...
    // {
    //
    //   widget.transactionType == "PO" && list1[i]['ItemPOBlocked'] != 1
    //       ? itemMasters.add(list1[i])
    //       : (widget.transactionType == "MJ" ||
    //       widget.transactionType == "TO") &&
    //       list1[i]['ItemInventBlocked'] != 1
    //       ? itemMasters.add(list1[i])
    //       : itemMasters.add(list1[i]);
    //
    //   // itemMasters.add(value);
    // }
    //
    // for(var i=1; i<=15;i++){
    //
    //   widget.transactionType == "PO" && list2[i]['ItemPOBlocked'] != 1
    //       ? itemMastersLists.add(list2[i])
    //       : (widget.transactionType == "MJ" ||
    //       widget.transactionType == "TO") &&
    //       list2[i]['ItemInventBlocked'] != 1
    //       ? itemMastersLists.add(list2[i])
    //       : itemMastersLists.add(list2[i]);
    //
    //   // itemMasters.add(value);
    // }
     ...
     print("Transaction type Line 1525 : ${widget.transactionType.runtimeType}");
